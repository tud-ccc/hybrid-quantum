...
module {
  ...
  // Ensures interleaved X does not prevent H cancellation.
  // CHECK-LABEL: func.func @interleaved_gates(
  func.func @interleaved_gates() -> !quantum.qubit<1> {
    // CHECK-DAG: %[[Q1:.+]] = "quantum.alloc"() : () -> !quantum.qubit<1>
    %q1 = "quantum.alloc"() : () -> (!quantum.qubit<1>)
  ...
  // Ensures a single H is not incorrectly removed.
  ...
  // Ensures a single X is not incorrectly removed.
